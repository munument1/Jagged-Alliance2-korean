...
2. 게임을 종료합니다.
3. 릴리즈 ZIP의 압축을 풉니다.
4. **이전 알파 버전을 설치한 적이 있다면 `install.ps1` 사용을 권장합니다.** 이전 빌드가 남긴 CIV 더미 EDT는 일반 덮어쓰기로 삭제되지 않기 때문입니다.
...
수동 설치가 필요하면 압축을 푼 폴더의 `Patch` 안에 있는 `Data`, `Data-1.13`, `ja2.exe`, `Ja2.ini`를 게임 설치 폴더에 그대로 복사하고 덮어쓸 수 있습니다. 다만 이전 알파의 삭제 대상 파일은 남을 수 있으므로 업그레이드 설치에는 권장하지 않습니다.

기존 `ja2.exe`와 `Ja2.ini`는 수동 덮어쓰기 전에 직접 백업하는 것을 권장합니다. 릴리즈의 `Ja2.ini`는 한국어 글꼴과 전체 화면 실행이 이미 설정되어 있습니다.
